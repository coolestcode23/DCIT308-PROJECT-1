...
public class Data {
    public static void populateSuppliers(Session session) {
    ...
    public static void populateDrugs(Session session) {
        Transaction transaction = session.beginTransaction();

        List<Supplier> suppliers = session.createQuery("from Supplier", Supplier.class).getResultList();
        Drug drug1 = new Drug(1, "Aspirin", "A common pain reliever and anti-inflammatory drug", 10.00, 100, 200, 500);
        Drug drug2 = new Drug(2, "Ibuprofen", "A nonsteroidal anti-inflammatory drug (NSAID) used for pain relief", 15.00, 50, 150, 400);
        ...
        drug1.addSupplier(suppliers.get(1));
        drug1.addSupplier(suppliers.get(2));
        drug1.addSupplier(suppliers.get(3));
        drug2.addSupplier(suppliers.get(0));
        drug2.addSupplier(suppliers.get(4));

        session.update(drug1);
        session.update(drug2);

        transaction.commit();
    }

    public static void populateCustomers(Session session) {
        Transaction transaction = session.beginTransaction();
    ...
    public static void populatePurchases(Session session) {
        Transaction transaction = session.beginTransaction();

        List<Drug> drugs = session.createQuery("from Drug", Drug.class).getResultList();
        List<Customer> customers = session.createQuery("from Customer", Customer.class).getResultList();

        Purchase purchase1 = new Purchase(1, drugs.get(0), customers.get(0), 3, LocalDateTime.now().minusDays(1));
        Purchase purchase2 = new Purchase(2, drugs.get(1), customers.get(1), 1, LocalDateTime.now().minusDays(2));
        Purchase purchase3 = new Purchase(3, drugs.get(2), customers.get(2), 5, LocalDateTime.now().minusDays(3));
        Purchase purchase4 = new Purchase(4, drugs.get(3), customers.get(3), 2, LocalDateTime.now().minusDays(4));
        Purchase purchase5 = new Purchase(5, drugs.get(4), customers.get(4), 7, LocalDateTime.now().minusDays(5));
        ...
        customer1.addPurchase(purchase3);
        customer2.addPurchase(purchase4);
        customer2.addPurchase(purchase5);
    ...
    public static void populateDatabase() {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            populateDrugs(session);
            populateCustomers(session);
            populatePurchases(session);
        }
    }
}
